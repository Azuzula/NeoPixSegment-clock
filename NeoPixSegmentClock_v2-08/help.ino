...
void zakladniNastaveni () {
  ...
  if (myStr[0] == "help") {
    ...
    Serial << endl;
    Serial << F("Pir:0, 1 [0] // Vypne(0)/zapne(1) PIR senzor") << endl;
    Serial << F("Svit:1 až 255 [8] // Doba v sec po kterou jsou hodiny rozsvícené od konce detekce pohybu. Tedy 2,5s + Svit") << endl;
    Serial << F("JasMod:10 až 255 [20] // Nastaví násobič jasu. Jak moc má displej svítit v závislosti na okolním osvětlení.") << endl;
    Serial << F("JasMin:1 až 255 [2] // Minimální jas.") << endl;
    Serial << F("JasMax:1 až 255 [240] // Maximální jas.") << endl;
    ...
    Serial << "minimum = " << minimum << endl;
    Serial << "maximum = " << maximum << endl;
    EEPROM.update(JAS_MIN, myStr[1].toInt());
    minimum =  myStr[1].toInt();
    if (minimum >= maximum) {
      EEPROM.update(JAS_MAX, myStr[1].toInt());
      maximum = myStr[1].toInt();
    }
    Jas_read();
    Serial << F("Hodnota JasMin nastavena na ") <<  myStr[1].toInt() << endl;
  }
  else if (myStr[0] == "jasmax") {
    EEPROM.update(JAS_MAX,  myStr[1].toInt());
    maximum =  myStr[1].toInt();
    if (maximum <= minimum) {
      EEPROM.update(JAS_MIN, myStr[1].toInt());
    ...
    Jas_read();
    Serial << F("Hodnota JasMax nastavena na ") <<  myStr[1].toInt() << endl;
  }
  else if (myStr[0] == "r") {
    ...
    Serial << F("JasMax = ") << EEPROM.read(JAS_MAX) << endl;
    ...
    Serial << F("Ujisti se, že zadáváš správný příkaz") << endl <<
           F("a pro odeslání příkazu máš nastaven jen NL (new line)") << endl;
  }
}
...
void RESET() {
  ...
  EEPROM.update(JAS_MAX, 240);
  EEPROM.update(COLOR_INDEX, 1);
  EEPROM.update(RED, 100);
  EEPROM.update(GREEN, 100);
  EEPROM.update(BLUE, 100);
  EEPROM.update(NOC,  0);
  EEPROM.update(NRED, 100);
  ...
  maximum = 240;
  colorIndex = EEPROM.read(COLOR_INDEX);
  myColor[colorIndex - 1][0] = 100;
  myColor[colorIndex - 1][1] = 100;
  myColor[colorIndex - 1][2] = 100;
  setColor();
