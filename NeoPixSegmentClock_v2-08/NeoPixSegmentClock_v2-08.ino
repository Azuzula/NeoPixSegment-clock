...
//***** night mode switch after X consequences reading
int nCounter = 0;
int nModeSW = 0;
byte nThreshold = 2;
...
void setup()
{
  ...
  //Serial << F("Yup") << endl;
  Serial << F("*************") << endl;
  Serial << F("NeoPixSegment hodiny ") << VERSION << endl;
  ...
  //Serial << F("Kapacitní snímače TTP223 před každým číslem i dvojtečkou") << endl;
  Serial << F("*************") << endl;
  Serial << F("Datum vytvoření FW a verze: ") << DATE << " " << VERSION << endl;
  ...
  Pix.Init(displayPin, LedsPerSegment, Format, 0);
  ...
  if (Jas <= nThreshold && (bool)EEPROM.read(NOC)) {
